...
WHERE step_id = (SELECT MAX(step_id) FROM `_71RESIDENT`.NP_NERA WHERE step = 1530 AND status = 0) AND STARE='activ'AND SUBCATEGORIE <> ''
ORDER BY SUBCATEGORIE
), sub_categorii_din_pt_manag_art as (
SELECT DISTINCT 
	ADMA_ALT_SUBCATEG
FROM `_71RESIDENT`.NP_NERA
WHERE status = 3 AND step = 1060 AND ADMA_ALT_SUBCATEG <> ''
)
